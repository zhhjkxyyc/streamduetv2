...
class CacheServer(BaseServer):
    ...
    def simulate_low_query(self, start_fid, end_fid, images_direc, results_dict, video_name,simulation=True, rpn_enlarge_ratio=0.0, extract_regions=True):
        if extract_regions:
        ...
        for single_result in batch_results.regions:
            ...
            #         single_result.label in self.relevant_classes and single_result.conf < self.config.low_threshold):
            #     detections.add_single_result(
            #         single_result, self.config.intersection_threshold)
            #     frame_image = frame_images[single_result.fid]
            #     if single_result.conf>=self.config.high_threshold:
