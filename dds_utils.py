...
class Region:
    ...
    def merge_regions(regions, iou_threshold=0.7):
        merged_regions = []
        used = [False] * len(regions)  # 用来记录是否已经处理过该Region
...
def crop_images(results, vid_name, images_direc, image_extension, resolution=None):
    ...
    for region in results.regions:
        cached_image = (region.fid, image_cache[region.fid])  # 从缓存中获取图像
...
#                                       f"{str(region.fid).zfill(8)}.{image_extension}")
...
#
#         cropped_image[y0:y1, x0:x1, :] = cached_image[1][y0:y1, x0:x1, :]
#         # 使用平滑滤波减少涂黑区域和周围区域的对比
#         # blurred_image = cv.GaussianBlur(cropped_image[y0:y1, x0:x1, :], (5, 5), 0)
#         # cropped_image[y0:y1, x0:x1, :] = blurred_image
...
def compute_regions_size(results, vid_name, images_direc, resolution, qp,
    ...
    if estimate_banwidth:
        ...
        # 记录 crop_images 的执行时间
        start_time = time.time()  # 开始时间
        frames_count = crop_images(results, vid_name, images_direc, image_extension, resolution)
        end_time = time.time()  # 结束时间
        crop_images_time = end_time - start_time  # 计算时间差
        print(f"crop_images executed in: {crop_images_time:.4f} seconds")

        # 记录 compress_and_get_size 的执行时间
        start_time = time.time()  # 开始时间
        size = compress_and_get_size(vid_name, 0, frames_count, qp=qp, enforce_iframes=enforce_iframes, resolution=1)
        end_time = time.time()  # 结束时间
        compress_size_time = end_time - start_time  # 计算时间差
        print(f"compress_and_get_size executed in: {compress_size_time:.4f} seconds")
