...
class TensorFlowDetector:
    ...
    def infer(self, image_np):
        ...
        for i in range(len(output_dict['detection_boxes'])):
            ...
            for k in TensorFlowDetector.classes.keys():
                if object_class in TensorFlowDetector.classes[k]:
                    object_class = k
                    relevant_class = True
                    break
        ...
        for idx_region, region in enumerate(output_dict['RPN_box_normalized']):
            ...
            if conf < TensorFlowDetector.rpn_threshold or w * h == 0.0 or w * h > 0.04:
                continue
